<template>
  <div class="flex gap-4 h-[calc(100vh-6rem)]">
    ...
    <!-- History sidebar -->
    <div
      class="w-72 bg-base-200 rounded-lg flex flex-col overflow-hidden shrink-0"
    >
      ...
      <div class="flex-1 overflow-auto">
        ...
        <div
          ...
          class="p-3 text-sm text-base-content/40"
        >
          No history yet
        </div>
      </div>
...
const selectedDs = computed(() =>
  datasources.value.find((d) => d.id === datasourceId.value),
);
...
const resultComponent = computed<Component>(() => {
  ...
  // Loki streams
  if (data?.data?.result?.[0]?.values && selectedDs.value?.type === "loki")
    return LogsPanel;
