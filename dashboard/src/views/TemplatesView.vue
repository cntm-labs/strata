  ...
  <div>
          ...
          >
            <div class="card-body p-4">
              ...
              <div class="flex gap-2 mt-2">
                <span v-if="tpl.required_datasource_type" class="badge badge-sm badge-outline">
                  {{ tpl.required_datasource_type }}
                </span>
                <span class="badge badge-sm badge-ghost">{{ panelCount(tpl) }} panels</span>
              </div>
            </div>
          </div>
        </div>
    ...
    >
      <form @submit.prevent="confirmUse" class="space-y-4">
        ...
        <div v-if="selectedTemplate?.required_datasource_type" class="form-control">
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" class="btn btn-ghost" @click="dialogVisible = false">Cancel</button>
...
function panelCount(tpl: DashboardTemplate) {
  const json = tpl.dashboard_json as Record<string, unknown> | null
  const panels = json?.panels
  return Array.isArray(panels) ? panels.length : 0
}

const filteredDatasources = computed(() => {
  if (!selectedTemplate.value?.required_datasource_type) return datasources.value
...
onMounted(async () => {
  try {
    const [tpls, dss] = await Promise.all([templatesApi.list(), datasourcesApi.list()])
