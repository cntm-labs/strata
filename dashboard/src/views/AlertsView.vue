<template>
  <div>
    ...
    <div class="tabs tabs-bordered mb-4">
      <a
        class="tab"
        :class="{ 'tab-active': tab === 'rules' }"
        @click="tab = 'rules'"
        >Rules</a
      >
      <a
        class="tab"
        :class="{ 'tab-active': tab === 'events' }"
        @click="tab = 'events'"
        >Events</a
      >
    </div>
        ...
        <Column header="Condition">
          <template #body="{ data }">
            <span class="font-mono text-sm"
              >{{ data.condition }} {{ data.threshold }}</span
            >
          </template>
        </Column>
        <Column field="severity" header="Severity">
          <template #body="{ data }">
            <span class="badge" :class="severityClass(data.severity)">{{
              data.severity
            }}</span>
          </template>
        </Column>
    ...
    <div v-if="tab === 'events'">
      ...
      >
        <Column header="State">
          <template #body="{ data }">
            <span class="badge" :class="stateClass(data.state)">{{
              data.state
            }}</span>
          </template>
        </Column>
...
function severityClass(severity: string) {
  return (
    { info: "badge-info", warning: "badge-warning", critical: "badge-error" }[
      severity
    ] || "badge-ghost"
  );
}
...
function stateClass(state: string) {
  return (
    { ok: "badge-success", firing: "badge-error", pending: "badge-warning" }[
      state
    ] || "badge-ghost"
  );
}
